...
export async function runPipeline(product, callbacks = {}) {
  ...
  try {
    // [1] 생성 (분석 + 작성 통합)
    onStage(0);
    const draft = await callClaude(buildGenerationPrompt(product), TOKENS.generation);
    onDraft(draft);
    onStage(2);
    ...
    const checkCompliance = (content) =>
      callClaude(buildCompliancePrompt(content, product.category), TOKENS.compliance);
 ...
 */
export async function regenerateSection(draft, idx, product) {
  const target =
    idx === "hero"
      ? { hero_headline: draft.hero_headline, hero_subcopy: draft.hero_subcopy }
      : draft.sections[idx];
  return callClaude(buildRegeneratePrompt(draft, target, product), TOKENS.regenerate);
}
